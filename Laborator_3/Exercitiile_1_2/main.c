...
#define CONFIG_FIRMWARE_VERSION_URL "https://192.168.89.43:5000/version"
...
int get_server_version()
{
    ...
    if (err == ESP_OK) {
        ...
        if (content_length > 0) {
            ...
            int server_version = atoi(response);
            ESP_LOGI(TAG, "Server version: %d", server_version);
...
static void ota_task(void *pvParameters)
{
    ...
    ESP_LOGI(TAG, "Checking server version...");
    int server_version = get_server_version();
    if (server_version == -1) {
        ESP_LOGE(TAG, "Failed to get server version, aborting OTA.");
        return;
    }
 
    int current_version = 3;  // deci aici trebuie sa dau import la VERSION din version.h
    ESP_LOGI(TAG, "Current version: %d, Server version: %d", current_version, server_version);
