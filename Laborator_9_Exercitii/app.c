 ...
 void sl_bt_on_event(sl_bt_msg_t *evt)
 {
   ...
   switch (SL_BT_MSG_ID(evt->header)) {
     ...
     case sl_bt_evt_system_boot_id:
       ...
       {
           app_log("Conexiuni inchise. Se sterg bound-urile.");
                  sl_bt_sm_delete_bondings();
       }
 
       // CONFIGURARE SECURITY MANAGER
       sc = sl_bt_sm_configure(0x03, sl_bt_sm_io_capability_displayonly);
       app_assert_status(sc);
 
       // Setare parola (passkey)
       sc = sl_bt_sm_set_passkey(123456);
       app_assert_status(sc);
 
       // Activare bonding
       sc = sl_bt_sm_store_bonding_configuration(1, 2); // bondable + overwrite if full
       app_assert_status(sc);
     ...
     case sl_bt_evt_sm_passkey_display_id:
       app_log("Parola pe care trebuie sa o introduceti este: %lu\r\n", evt->data.evt_sm_passkey_display.passkey);
       break;
 
     // Bonding reușit
     case sl_bt_evt_sm_bonded_id:
       app_log("Asociere reușită! Dispozitivele sunt acum conectate.\r\n");
       break;
 
     // Bonding eșuat
     case sl_bt_evt_sm_bonding_failed_id:
       app_log("Eroare în bonding. Cod: 0x%04X\r\n", evt->data.evt_sm_bonding_failed.reason);
       break;
 
     // Nivel de securitate
     case sl_bt_evt_connection_parameters_id:
       app_log("Nivel de securitate conexiune: %d\r\n", evt->data.evt_connection_parameters.security_mode);
       break;
